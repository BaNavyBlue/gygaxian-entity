...
<img src="https://github.com/BaNavyBlue/gygaxian-entity/blob/main/gygax_party.png" alt="TUI panel 2" width="1000"/>
...
items/Arms.json is dynamically added into a vector of shared Item pointers. The character has a vector of Items, only the items ID, TYPE and count are stored in the Character .json since the list of item parameters can get quite large. 
...
I've been hacking around and modifying some things. I disabled the CPP ifdef flags and cpp code in the rogueutil header as I was observing some things not clearing properly and garbage characters in the example code.
...
Accidentally painted myself into a monlithic main/UI source file corner using the header file for rogueutil.h since the implementation is in the header I was unable to share the header between modules without creating re-definition errors.  I moved the header implementation into a .c file. 
...
# Another Goal?
When I might try to write a character sheet generator which generates laTex code.
