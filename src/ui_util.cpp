...
void TextInput::createTextInput(DrawRange uRandWidth, Perimeter inPerim,
                                std::string message) {
  ...
  drawSmall(_textScreen->xyLimits.minX, _textScreen->xyLimits.maxX,
            _textScreen->xyLimits.minY, _textScreen->xyLimits.maxY + 1,
            *_textScreen);
  getText();
}
...
void OptionWindow::PlaceOptions() {
  for (std::size_t i = _optScreen->xyLimits.minY;
       i < _optScreen->xyLimits.maxY + 1; ++i) {
    for (std::size_t j = _optScreen->xyLimits.minX;
         j < _optScreen->xyLimits.maxX + 1; ++j) {
      if ((i == _optScreen->xyLimits.minY + 1) &&
          j == _optScreen->xyLimits.minX + 3) {
  ...
  drawSmall(_optScreen->xyLimits.minX, _optScreen->xyLimits.maxX,
            _optScreen->xyLimits.minY, _optScreen->xyLimits.maxY + 1,
            *_optScreen);
}
...
// {
//     for(std::size_t i = _optScreen->xyLimits.minY; i <
//     _optScreen->xyLimits.maxY + 1; ++i){
//         for(std::size_t j = _optScreen->xyLimits.minX; j <
//         _optScreen->xyLimits.maxX + 1; ++j){
//             if((i == _optScreen->xyLimits.minY + 1) && j ==
//             _optScreen->xyLimits.minX + 3){
//                 for(int m = 0; m < _list.size(); ++m){
//                     for(int n = 0; n < _list[m].size(); ++n){
//                         _optScreen->charMap[i][j] = _list[m][n];
//                         _optScreen->colorMap[i][j] = YELLOW;
...
PrintInfo::PrintInfo(Entity chrctr, DrawRange uRandWidth, Perimeter inPerim,
  ...
  for (int i = 0; i < _infoScreen.size(); ++i) {
    drawSmall(_infoScreen[i]->xyLimits.minX, _infoScreen[i]->xyLimits.maxX,
              _infoScreen[i]->xyLimits.minY, _infoScreen[i]->xyLimits.maxY + 1,
              *_primaryScreen);
  }
  _infoScreen.clear();
}
...
void PrintInfo::PlaceInfo(int vectIdx) {
  for (std::size_t i = _infoScreen[vectIdx]->xyLimits.minY;
       i < _infoScreen[vectIdx]->xyLimits.maxY + 1; ++i) {
    for (std::size_t j = _infoScreen[vectIdx]->xyLimits.minX;
         j < _infoScreen[vectIdx]->xyLimits.maxX + 1; ++j) {
      if ((i == _infoScreen[vectIdx]->xyLimits.minY + 1) &&
          j == _infoScreen[vectIdx]->xyLimits.minX + 3) {
...
void PrintInfo::MakeGenTbl(std::vector<std::string> stringVect) {
  ...
  _infoScreen[scrnIdx]->xyLimits.maxX =
      _infoScreen[scrnIdx]->xyLimits.minX + maxLen + 7;
  _infoScreen[scrnIdx]->xyLimits.maxY =
      _infoScreen[scrnIdx]->xyLimits.minY + stringVect.size() + 1;
  ...
  drawSmall(_infoScreen[scrnIdx]->xyLimits.minX,
            ...
            _infoScreen[scrnIdx]->xyLimits.maxY + 1, *_infoScreen[scrnIdx]);
  ...
  drawSmall(_infoScreen[scrnIdx]->xyLimits.minX,
            _infoScreen[scrnIdx]->xyLimits.maxX,
            _infoScreen[scrnIdx]->xyLimits.minY,
            _infoScreen[scrnIdx]->xyLimits.maxY + 1, *_primaryScreen);
...
void PrintInfo::MakeCharWin() {
  ...
  drawSmall(_infoScreen[0]->xyLimits.minX, _infoScreen[0]->xyLimits.maxX,
            _infoScreen[0]->xyLimits.minY, _infoScreen[0]->xyLimits.maxY + 1,
            *_infoScreen[0]);
}
...
void PrintInfo::MakeStatsWin() {
  ...
  basicInfo.push_back("(S)trength: " +
                      std::to_string(_character->getStats().strength));
  if (_character->getStats().strength > 17 &&
      (_character->getClass()[0] == FIGHTER ||
       _character->getClass()[0] == PALADIN ||
       _character->getClass()[0] == RANGER)) {
    basicInfo.push_back("Exceptional Strength: " +
                        std::to_string(_character->getStats().excStren));
  }
  basicInfo.push_back("(I)ntelligence: " +
                      std::to_string(_character->getStats().intelligence));
  basicInfo.push_back("(W)isdom: " +
                      std::to_string(_character->getStats().wisdom));
  basicInfo.push_back("(D)exterity: " +
                      std::to_string(_character->getStats().dexterity));
  basicInfo.push_back("C(o)nstitution: " +
                      std::to_string(_character->getStats().constitution));
  basicInfo.push_back("C(h)arisma: " +
                      std::to_string(_character->getStats().charisma));
  if (_character->getRace() == DWARF || _character->getRace() == HALF_ORC) {
    basicInfo.back() +=
        "(*" + std::to_string(_character->getStats().raceCharisma) + ")";
  ...
  _infoScreen[1]->xyLimits.maxY =
      _infoScreen[0]->xyLimits.minY + basicInfo.size() + 1;
  ...
  drawSmall(_infoScreen[1]->xyLimits.minX, _infoScreen[1]->xyLimits.maxX,
            _infoScreen[1]->xyLimits.minY, _infoScreen[1]->xyLimits.maxY + 1,
            *_infoScreen[1]);
}
...
void PrintInfo::MakeCmbtWin() {
  ...
  char varIn[32];
  basicInfo.back() += "(" + std::to_string(_character->getCurHP()) + "/" +
                      std::to_string(_character->getMaxHP()) + ")";
  ...
  _infoScreen.push_back(
      std::make_shared<ScreenVals>(VECT_MAX, ' ', YELLOW, BLACK));
  _infoScreen[2]->xyLimits.minX = healCom.minX =
      _infoScreen[1]->xyLimits.maxX + 1;
  _infoScreen[2]->xyLimits.minY = healCom.minY = _infoScreen[1]->xyLimits.minY;
  _infoScreen[2]->xyLimits.maxX = healCom.maxX =
      _infoScreen[1]->xyLimits.maxX + maxLen + 7;
  _infoScreen[2]->xyLimits.maxY = healCom.maxY =
      _infoScreen[1]->xyLimits.minY + basicInfo.size() + 1;
  generatePerimeter(*_infoScreen[2], _infoBoxPerim);
  _contents.push_back(basicInfo);

  PlaceInfo(2);
  drawSmall(_infoScreen[2]->xyLimits.minX, _infoScreen[2]->xyLimits.maxX,
            _infoScreen[2]->xyLimits.minY, _infoScreen[2]->xyLimits.maxY + 1,
            *_infoScreen[2]);
}
...
void PrintInfo::MakeWealthWin() {
  ...
  basicInfo.push_back("Silver: " +
                      std::to_string(_character->getMoney().silver));
  basicInfo.push_back("Copper: " +
                      std::to_string(_character->getMoney().copper));
  basicInfo.push_back("Electrum: " +
...
void PrintInfo::MakeLangWin() {
  ...
  _infoScreen.push_back(
      std::make_shared<ScreenVals>(VECT_MAX, ' ', YELLOW, BLACK));
  _infoScreen[4]->xyLimits.minX = healCom.minX =
      _infoScreen[3]->xyLimits.maxX + 1;
  _infoScreen[4]->xyLimits.minY = healCom.minY = _infoScreen[3]->xyLimits.minY;
  _infoScreen[4]->xyLimits.maxX = healCom.maxX =
      _infoScreen[3]->xyLimits.maxX + maxLen + 7;
  _infoScreen[4]->xyLimits.maxY = healCom.maxY =
      _infoScreen[3]->xyLimits.minY + basicInfo.size() + 1;
  generatePerimeter(*_infoScreen[4], _infoBoxPerim);
  _contents.push_back(basicInfo);

  PlaceInfo(4);
  drawSmall(_infoScreen[4]->xyLimits.minX, _infoScreen[4]->xyLimits.maxX,
            _infoScreen[4]->xyLimits.minY, _infoScreen[4]->xyLimits.maxY + 1,
            *_infoScreen[4]);
}
...
void PrintInfo::MakeDexTbl() {
  ...
  bool isThief = (_character->getClass()[0] == THIEF ||
                  _character->getClass()[0] == ASSASSIN);

  DrawRange healCom;
  int screenIdx = _infoScreen.size();
  _infoScreen.push_back(
      std::make_shared<ScreenVals>(VECT_MAX, ' ', YELLOW, BLACK));
  _infoScreen[screenIdx]->xyLimits.minX = healCom.minX =
      _infoScreen[0]->xyLimits.minX + 1;
  _infoScreen[screenIdx]->xyLimits.minY = healCom.minY =
      _infoScreen[0]->xyLimits.minY + 1;
  _infoScreen[screenIdx]->xyLimits.maxX = healCom.maxX =
      _infoScreen[0]->xyLimits.minX + maxLen + 7;
  _infoScreen[screenIdx]->xyLimits.maxY = healCom.maxY =
      _infoScreen[0]->xyLimits.minY + basicInfo.size() + 2;
  ...
  drawSmall(_infoScreen[screenIdx]->xyLimits.minX,
            ...
            _infoScreen[screenIdx]->xyLimits.maxY + 1, *_infoScreen[screenIdx]);
  ...
  if (isThief) {
    ...
    _infoScreen.push_back(
        std::make_shared<ScreenVals>(VECT_MAX, ' ', YELLOW, BLACK));
    _infoScreen[screenIdx]->xyLimits.minX = healCom.minX =
        _infoScreen[screenIdx - 1]->xyLimits.maxX + 1;
    _infoScreen[screenIdx]->xyLimits.minY = healCom.minY =
        _infoScreen[0]->xyLimits.minY + 1;
    _infoScreen[screenIdx]->xyLimits.maxX = healCom.maxX =
        _infoScreen[screenIdx]->xyLimits.minX + maxLen + 7;
    _infoScreen[screenIdx]->xyLimits.maxY = healCom.maxY =
        _infoScreen[screenIdx]->xyLimits.minY + basicInfo.size() + 2;
    generatePerimeter(*_infoScreen[screenIdx], _infoBoxPerim);
    _contents.push_back(basicInfo);
  ...
  drawSmall(_infoScreen[screenIdx]->xyLimits.minX,
            _infoScreen[screenIdx]->xyLimits.maxX,
            _infoScreen[screenIdx]->xyLimits.minY,
            _infoScreen[screenIdx]->xyLimits.maxY + 1, *_primaryScreen);
...
void PrintInfo::MakeRaceSkills() {
  std::vector<std::string> basicInfo =
      _character->getRaceSkills()->getRaceSkills();
  ...
  _infoScreen[10]->xyLimits.minY = healCom.minY =
      _infoScreen[8]->xyLimits.maxY + 1;
  _infoScreen[10]->xyLimits.maxX = healCom.maxX =
      _infoScreen[0]->xyLimits.minX + maxLen + 7;
  _infoScreen[10]->xyLimits.maxY = healCom.maxY =
      _infoScreen[8]->xyLimits.maxY + basicInfo.size() + 2;
  generatePerimeter(*_infoScreen[10], _infoBoxPerim);
  _contents.push_back(basicInfo);

  PlaceInfo(10);
  drawSmall(_infoScreen[10]->xyLimits.minX, _infoScreen[10]->xyLimits.maxX,
            _infoScreen[10]->xyLimits.minY, _infoScreen[10]->xyLimits.maxY + 1,
            *_infoScreen[10]);
}
...
void PrintInfo::drawPrimary() {
  ...
  for (std::size_t i = 0; i < _vert; ++i) {
    for (std::size_t j = 0; j < _horz; ++j) {
      ...
      colorPrintUTF(_primaryScreen->colorMap[i][j],
                    _primaryScreen->bGColorMap[i][j], utfChar.c_str());
    }
  }
  locate(1, 1);
  fflush(stdout);
...
WarnMessage::WarnMessage(std::string warning, std::string question) {
  ...
  Perimeter warnPerim(RADIOACT, RADIOACT, RADIOACT, RADIOACT, INV_PENT,
                      INV_PENT, YELLOW, BLACK, border, BLACK);
  ...
  for (int i = 0; i < warning.size(); ++i) {
    _warnScreen->charMap[boxCorners.minY + 3][boxCorners.minX + 5 + i] =
        warning[i];
    _warnScreen->colorMap[boxCorners.minY + 3][boxCorners.minX + 5 + i] =
        border;
    _warnScreen->bGColorMap[boxCorners.minY + 3][boxCorners.minX + 5 + i] =
        BLACK;
  }

  for (int i = 0; i < question.size(); ++i) {
    _warnScreen->charMap[boxCorners.minY + 5][boxCorners.minX + 5 + i] =
        question[i];
    _warnScreen->colorMap[boxCorners.minY + 5][boxCorners.minX + 5 + i] =
        border;
    _warnScreen->bGColorMap[boxCorners.minY + 5][boxCorners.minX + 5 + i] =
        BLACK;
  }

  drawSmall(boxCorners.minX, boxCorners.maxX, boxCorners.minY,
            boxCorners.maxY + 1, *_warnScreen);
}
...
void drawSmall(int startX, int maxX, int startY, int maxY,
  ...
  for (int i = startY; i < maxY; ++i) {
    for (int j = startX; j < maxX + 1; ++j) {
      ...
      // printf("%s\n", utfChar.c_str());
      colorPrintUTF(inScreen.colorMap.at(i).at(j),
                    inScreen.bGColorMap.at(i).at(j), utfChar.c_str());
      // } else {
      //     char singleChar[2];
...
std::string getUTF(int inCode) {
  ...
  // sscanf(unicodeEscape + 2, "%x", &codePoint);  // Skip the "\u" part and
  ...
  case 1:
    ...
    return utf8Result;
  case 2:
    sprintf(utf8Result, "%c%c", (char)(0xC0 | (inCode >> 6)),
            (char)(0x80 | (inCode & 0x3F)));
    utf8Result[4] = '\0';
    return utf8Result;
  case 3:
    sprintf(utf8Result, "%c%c%c", (char)(0xE0 | (inCode >> 12)),
            (char)(0x80 | ((inCode >> 6) & 0x3F)),
            (char)(0x80 | (inCode & 0x3F)));
    utf8Result[4] = '\0';
    return utf8Result;
  case 4:
    sprintf(utf8Result, "%c%c%c%c", (char)(0xF0 | (inCode >> 18)),
            (char)(0x80 | ((inCode >> 12) & 0x3F)),
            (char)(0x80 | ((inCode >> 6) & 0x3F)),
            (char)(0x80 | (inCode & 0x3F)));
    utf8Result[4] = '\0';
    return utf8Result;
...
void generatePerimeter(ScreenVals &inScreen, Perimeter inPerim) {
  for (std::size_t i = inScreen.xyLimits.minY; i < inScreen.xyLimits.maxY + 1;
       ++i) {
    for (std::size_t j = inScreen.xyLimits.minX; j < inScreen.xyLimits.maxX + 1;
         ++j) {
      ...
      } else if ((j == inScreen.xyLimits.minX || j == inScreen.xyLimits.maxX) &&
                 (i < inScreen.xyLimits.maxY)) {
...
ChooseOpt::ChooseOpt(std::vector<std::string> inOpts) {
  ...
  strtCrn.minX = 1;
  strtCrn.minY = 1;
  Perimeter inPerim(0x256D, 0x256E, 0x2570, 0x256F, 0x2500, 0x2502, MAGENTA,
                    BLACK, BLUE, BLACK);
...
RollScreen::RollScreen(stats inStats, ScreenVals &primaryScreen) {
  ...
  Perimeter rollPerim(0x256D, 0x256E, 0x2570, 0x256F, 0x2500, 0x2502, MAGENTA,
                      BLACK, BLUE, BLACK);
  ...
  drawSmall(1, rollScreen->xyLimits.maxX, 1, rollScreen->xyLimits.maxY,
            *rollScreen);
}
...
ListHighlight::ListHighlight(std::vector<std::string> inList,
...
//             _listScreen->xyLimits.minX = _cornerDims.minX =
//             _primaryScreen->xyLimits.minX + 1; _listScreen->xyLimits.minY =
//             _cornerDims.minY = _primaryScreen->xyLimits.minY + 1;
...
//                 //     infoRange.minX = 1;
//                 //     infoRange.minY = 1;
//                 //     Perimeter rollPerim(0x256D, 0x256E, 0x2570, 0x256F,
//                 0x2500, 0x2502, MAGENTA, BLACK, BLUE, BLACK);
//                 //     PrintInfo showChar(_players.at(_currPos), infoRange,
//                 rollPerim, *_primaryScreen, tcols(), trows());
//                 //     listNavigate();
//                 //     createListPerimeter(*_destListScreen, _destOptions);
//                 //     createListScreen(*_destListScreen, _destList,
//                 _destTitle, _highlightDest);
//                 //     drawSmall(_cornerDims.minX, _cornerDims.maxX,
//                 _cornerDims.minY, _cornerDims.maxY + 1, *_listScreen);
//                 //     drawSmall(_destListScreen->xyLimits.minX,
//                 _destListScreen->xyLimits.maxX,
//                 //               _destListScreen->xyLimits.minY,
//                 _destListScreen->xyLimits.maxY + 1,
//                 //               *_destListScreen);
//                 // }
//             } else if(k == KEY_UP){
//                 if(_currPos > 0){
...
//             //             if(_destList.size() > 0){
//             //                 drawSmall(_cornerDims.minX, _cornerDims.maxX,
//             _cornerDims.minY, _cornerDims.maxY + 1, *_primaryScreen);
//             //                 drawSmall(_destListScreen->xyLimits.minX,
//             _destListScreen->xyLimits.maxX,
//             //                         _destListScreen->xyLimits.minY,
//             _destListScreen->xyLimits.maxY + 1,
//             //                         *_primaryScreen);
//             //                 TextInput partyName;
//             //                 Perimeter textPerim(TL_LINE, TR_LINE, BL_LINE,
//             BR_LINE, HORZ_LINE, VERT_LINE, MAGENTA, BLACK, BLUE, BLACK);
...
//             //                 do{
//             //                     confirmName = false;
//             // partyName.createTextInput(_cornerDims,textPerim,"Party
//             Name:");
//             // if(doesRecordExist(partyName.getAquiredString(), "parties/",
//             ".json")){
//             //                         WarnMessage nameExists("Party name
//             already in use", "Overwrite? (y/n)");
//             //                         confirmName =
//             nameExists.waitForAnswer();
//             // drawSmall(nameExists.getScreen().xyLimits.minX,
//             nameExists.getScreen().xyLimits.maxX,
//             nameExists.getScreen().xyLimits.minY,
//             nameExists.getScreen().xyLimits.maxY + 1, *_primaryScreen);
//             //                     }
...
//             //                 SaveParty saveMe(partyName.getAquiredString(),
//             memberPaths);
//             //                 drawSmall(partyName.getScreen().xyLimits.minX,
//             partyName.getScreen().xyLimits.maxX,
//             partyName.getScreen().xyLimits.minY,
//             partyName.getScreen().xyLimits.maxY + 1, *_primaryScreen);
//             //                 break;
//             //             }
...
//                 // partyName.createTextInput(_cornerDims,textPerim,"Party
//                 Name:");
//                 //         if(doesRecordExist(partyName.getAquiredString(),
//                 "parties/", ".json")){
//                 //             WarnMessage nameExists("Party name already in
//                 use", "Overwrite? (y/n)");
...
void ListHighlight::createListScreen(ScreenVals &inScreen,
...
void ListHighlight::createListPerimeter(ScreenVals &inScreen,
    ...
    for (int j = inScreen.xyLimits.minX; j <= inScreen.xyLimits.maxX; ++j) {
      if (i == inScreen.xyLimits.minY && j == inScreen.xyLimits.minX) {
        ...
        inScreen.colorMap[i][j] = _perim.cornerColr;
                 ...
                 j == inScreen.xyLimits.minX) {
        inScreen.charMap[i][j] = _perim.leftTee;
        inScreen.colorMap[i][j] = _perim.cornerColr;
      } else if (i == inScreen.xyLimits.minY + 2 &&
                 j == inScreen.xyLimits.maxX) {
        ...
        inScreen.colorMap[i][j] = _perim.cornerColr;
      } else if (j == inScreen.xyLimits.minX &&
                 i < inScreen.xyLimits.maxY - 1) {
        inScreen.charMap[i][j] = _perim.sideWalls;
        inScreen.colorMap[i][j] = _perim.cornerColr;
      } else if (j == inScreen.xyLimits.maxX &&
                 i < inScreen.xyLimits.maxY - 1) {
        inScreen.charMap[i][j] = _perim.sideWalls;
        inScreen.colorMap[i][j] = _perim.cornerColr;
      } else if (i == inScreen.xyLimits.maxY - 1) {
        ...
        inScreen.colorMap[i][j] = _perim.cornerColr;
                 ...
                 j - inScreen.xyLimits.minX < inOpts.size()) {
...
ListHighlightPair::ListHighlightPair(
    std::vector<std::string> &inList, std::vector<Entity> &inPlayers,
...
void ListHighlightPair::navigateSelection() {
  ...
  while (true) {
    ...
    if (horz_char != new_horz || vert_char != new_vert) {
      ...
      _listScreen->xyLimits.minX = _cornerDims.minX =
          _primaryScreen->xyLimits.minX + 1;
      _listScreen->xyLimits.minY = _cornerDims.minY =
          _primaryScreen->xyLimits.minY + 1;
      _listScreen->xyLimits.maxX = _cornerDims.maxX = horz_char / 3;
      ...
      _destListScreen->xyLimits.maxX = _listScreen->xyLimits.maxX * 2 + 1;
      (maxY >= 0) ? _destListScreen->xyLimits.maxY = maxY
                  : _destListScreen->xyLimits.maxY = maxY;
    ...
    if (kbhit()) {
      ...
      if (std::tolower(k) == 'a') {
        ...
        if (_players.size() > 0) {
          infoRange.minX = 1;
          infoRange.minY = 1;
          Perimeter rollPerim(0x256D, 0x256E, 0x2570, 0x256F, 0x2500, 0x2502,
                              MAGENTA, BLACK, BLUE, BLACK);
          PrintInfo showChar(_players.at(_currPos), infoRange, rollPerim,
                             *_primaryScreen, tcols(), trows());
          listNavigate();
          createListPerimeter(*_destListScreen, _destOptions);
          createListScreen(*_destListScreen, _destList, _destTitle,
                           _highlightDest);
        ...
        if (_destList.size() > 0) {
          ...
          if (std::tolower(k) == 's') {
            if (_destList.size() > 0) {
              drawSmall(_cornerDims.minX, _cornerDims.maxX, _cornerDims.minY,
                        _cornerDims.maxY + 1, *_primaryScreen);
              drawSmall(_destListScreen->xyLimits.minX,
                        _destListScreen->xyLimits.maxX,
                        _destListScreen->xyLimits.minY,
                        _destListScreen->xyLimits.maxY + 1, *_primaryScreen);
              TextInput partyName;
              Perimeter textPerim(TL_LINE, TR_LINE, BL_LINE, BR_LINE, HORZ_LINE,
                                  VERT_LINE, MAGENTA, BLACK, BLUE, BLACK);
              ...
              do {
                confirmName = false;
                partyName.createTextInput(_cornerDims, textPerim,
                                          "Party Name:");
                if (doesRecordExist(partyName.getAquiredString(), "parties/",
                                    ".json")) {
                  WarnMessage nameExists("Party name already in use",
                                         "Overwrite? (y/n)");
                  confirmName = nameExists.waitForAnswer();
                  drawSmall(nameExists.getScreen().xyLimits.minX,
                            nameExists.getScreen().xyLimits.maxX,
                            nameExists.getScreen().xyLimits.minY,
                            nameExists.getScreen().xyLimits.maxY + 1,
                            *_primaryScreen);
                }
              ...
              drawSmall(partyName.getScreen().xyLimits.minX,
                        partyName.getScreen().xyLimits.maxX,
                        partyName.getScreen().xyLimits.minY,
                        partyName.getScreen().xyLimits.maxY + 1,
                        *_primaryScreen);
              break;
            }
          } else if (k == KEY_ESCAPE) {
    ...
    if (kbhit()) {
      ...
      if (std::tolower(k) == 'r') {
        ...
        if (_destList.size() > 0) {
          ...
          Perimeter rollPerim(0x256D, 0x256E, 0x2570, 0x256F, 0x2500, 0x2502,
                              MAGENTA, BLACK, BLUE, BLACK);
          PrintInfo showChar(_players.at(_destIdx[_destCurrPos]), infoRange,
                             rollPerim, *_primaryScreen, tcols(), trows());
          listNavigate();
          destNavigate();
          // createListPerimeter(*_destListScreen, _destOptions);
        ...
        //     drawSmall(inScreen.xyLimits.minX, inScreen.xyLimits.maxX,
        //     inScreen.xyLimits.minY, inScreen.xyLimits.maxY, primaryScreen);
        return KEY_ESCAPE;
      }
...
void ListHighlightPair::listNavigate() {
  ...
  int titlePos = _listScreen->xyLimits.maxX -
                 (_listScreen->xyLimits.maxX - _listScreen->xyLimits.minX) / 2 -
                 _title.size() / 2 - 1;
  if (titlePos < 0)
    titlePos = 0;

  for (int i = 0; i < _title.size(); ++i) {
    _listScreen->charMap[_listScreen->xyLimits.minY + 1][i + titlePos] =
        _title[i];
    _listScreen->colorMap[_listScreen->xyLimits.minY + 1][i + titlePos] =
        YELLOW;
  }

  for (int i = _listScreen->xyLimits.minY + 3;
       i < _listScreen->xyLimits.maxY - 1; ++i) {
...
void ListHighlightPair::destNavigate() {
  ...
  int titlePos =
      _destListScreen->xyLimits.maxX -
      (_destListScreen->xyLimits.maxX - _destListScreen->xyLimits.minX) / 2 -
      _destTitle.size() / 2 - 1;
  if (titlePos < 0)
    titlePos = 0;

  for (int i = 0; i < _destTitle.size(); ++i) {
    _destListScreen->charMap[_destListScreen->xyLimits.minY + 1][i + titlePos] =
        _destTitle[i];
    _destListScreen
        ->colorMap[_destListScreen->xyLimits.minY + 1][i + titlePos] = YELLOW;
  }

  for (int i = _destListScreen->xyLimits.minY + 3;
       i < _destListScreen->xyLimits.maxY - 1; ++i) {
    ...
    if (strdx < _destList.size()) {
      for (int j = 0; j < _destList[strdx].size(); ++j) {
        if (strdx == _destCurrPos && _highlightDest) {
          _destPrevPos = _destCurrPos;
          _destListScreen->charMap[i][j + _destListScreen->xyLimits.minX + 1] =
              _destList[strdx][j];
          _destListScreen->colorMap[i][j + _destListScreen->xyLimits.minX + 1] =
              textCol;
          _destListScreen
              ->bGColorMap[i][j + _destListScreen->xyLimits.minX + 1] = bgCol;
        } else {
          _destListScreen->charMap[i][j + _destListScreen->xyLimits.minX + 1] =
              _destList[strdx][j];
          _destListScreen->colorMap[i][j + _destListScreen->xyLimits.minX + 1] =
              YELLOW;
          _destListScreen
              ->bGColorMap[i][j + _destListScreen->xyLimits.minX + 1] = BLACK;
        }
      }
    }
  }
...
ListHighlightPartySelect::ListHighlightPartySelect(
    std::vector<std::string> &inList,
    std::vector<std::vector<Entity>> &inParties,
    std::vector<std::string> &inPaths, ScreenVals &primaryScreen,
    std::string inName, std::vector<int> inOptions, Perimeter inPerim,
    DrawRange inRange) {
  _perim = inPerim;
  _list = inList;
  _pathList = inPaths;
  ...
  drawSmall(_partyListScreen->xyLimits.minX, _partyListScreen->xyLimits.maxX,
            _partyListScreen->xyLimits.minY,
            _partyListScreen->xyLimits.maxY + 1, *_partyListScreen);
...
void ListHighlightPartySelect::navigateSelection() {
  ...
  while (true) {
    ...
    if (horz_char != new_horz || vert_char != new_vert) {
      ...
      drawSmall(_cornerDims.minX, _cornerDims.maxX, _cornerDims.minY,
                _cornerDims.maxY + 1, *_listScreen);
      drawSmall(_partyListScreen->xyLimits.minX,
                _partyListScreen->xyLimits.maxX,
                _partyListScreen->xyLimits.minY,
                _partyListScreen->xyLimits.maxY + 1, *_partyListScreen);
    }
...
void ListHighlightPartySelect::populatePartyList() {
  ...
  int titlePos =
      _partyListScreen->xyLimits.maxX -
      (_partyListScreen->xyLimits.maxX - _partyListScreen->xyLimits.minX) / 2 -
      partyTitle.size() / 2 - 1;
  if (titlePos < 0)
    titlePos = 0;
  for (int i = 0; i < partyTitle.size(); ++i) {
    _partyListScreen
        ->charMap[_partyListScreen->xyLimits.minY + 1][i + titlePos] =
        partyTitle[i];
    _partyListScreen
        ->colorMap[_partyListScreen->xyLimits.minY + 1][i + titlePos] = YELLOW;
  }

  for (int i = _partyListScreen->xyLimits.minY + 3;
       i < _partyListScreen->xyLimits.maxY - 1; ++i) {
    // int idxOffset = _currPos - _listScreen->xyLimits.maxY + 6;

    int curMemb = i - (_partyListScreen->xyLimits.minY + 3);
    if (curMemb < 0)
      curMemb = 0;
    // std::cout << "_party.size(): " << _party.size() << " curMemb: " <<
    // curMemb << std::endl;
    if (curMemb < _party.size()) {
      // std::cout << curMemb << std::endl;
      for (int j = 0; j < _party[curMemb].size(); ++j) {

        _partyListScreen->charMap[i][j + _partyListScreen->xyLimits.minX + 1] =
            _party[curMemb][j];
        _partyListScreen->colorMap[i][j + _partyListScreen->xyLimits.minX + 1] =
            YELLOW;
        _partyListScreen
            ->bGColorMap[i][j + _partyListScreen->xyLimits.minX + 1] = BLACK;
      }
    }
  }
...
void ListHighlightPartySelect::formatSelectedParty() {
  ...
  for (int i = 0; i < _parties[_currPos].size(); ++i) {
    ...
    std::string member =
        curMem.getName() + ", Lvl: " + std::to_string(curMem.getLevel());
    member += ", Class: " + getClass(curMem.getClass()[0]) +
              " Align: " + getAlign(curMem.getAlign());
...
ListHighlightProfSelect::ListHighlightProfSelect(Entity &inChar,
                                                 ScreenVals &primaryScreen,
                                                 std::vector<int> inOptions[2],
                                                 Perimeter inPerim,
                                                 DrawRange inRange) {
  ...
  _profAvailable = _player->getCharClassSkills()[0]->getInitNumWeap() +
                   (_player->getLevel() - 1) /
                       _player->getCharClassSkills()[0]->getAddedWeapLevDen() -
                   _profChosen;
  ...
  navigateSelection();
  drawSmall(_listScreen->xyLimits.minX, _listScreen->xyLimits.maxX,
            _listScreen->xyLimits.minY, _listScreen->xyLimits.maxY,
            *_primaryScreen);
  drawSmall(_playerDestScreen->xyLimits.minX, _playerDestScreen->xyLimits.maxX,
            _playerDestScreen->xyLimits.minY, _playerDestScreen->xyLimits.maxY,
            *_primaryScreen);
  drawSmall(_descriptionPanel->xyLimits.minX, _descriptionPanel->xyLimits.maxX,
            _descriptionPanel->xyLimits.minY, _descriptionPanel->xyLimits.maxY,
            *_primaryScreen);
...
void ListHighlightProfSelect::createDescription(profData profSel) {
  ...
  for (int i = 0; i < _descriptionPanel->xyLimits.maxX; ++i) {
    if (i < description.size()) {
      _descriptionPanel->charMap[_descriptionPanel->xyLimits.minY + 1][i + 1] =
          description[i];
      _descriptionPanel->colorMap[_descriptionPanel->xyLimits.minY + 1][i + 1] =
          YELLOW;
    } else {
      _descriptionPanel->charMap[_descriptionPanel->xyLimits.minY + 1][i + 1] =
          ' ';
  ...
  //             if(strdx == 0 && highlight){
  //                 inScreen.charMap[i][j + inScreen.xyLimits.minX + 1] =
  //                 inList[strdx][j]; inScreen.colorMap[i][j +
  //                 inScreen.xyLimits.minX + 1] = textCol;
  //                 inScreen.bGColorMap[i][j + inScreen.xyLimits.minX + 1] =
  //                 bgCol;
  //             } else {
  //                 inScreen.charMap[i][j + inScreen.xyLimits.minX + 1] =
  //                 inList[strdx][j]; inScreen.colorMap[i][j +
  //                 inScreen.xyLimits.minX + 1] = YELLOW;
  //             }
  //         }
  //     }
  // }
          ...
          if (std::tolower(k) == 's') {
            if (_playerList.size() > 0) {
              ...
              drawSmall(_playerDestScreen->xyLimits.minX,
                        _playerDestScreen->xyLimits.maxX,
                        _playerDestScreen->xyLimits.minY,
                        _playerDestScreen->xyLimits.maxY + 1, *_primaryScreen);
...
char ListHighlightProfSelect::navigatePlayerDest() {
  char choice;
  std::size_t horz_char = tcols();
  std::size_t vert_char = trows();
  ...
  while (true) {
    ...
    if (horz_char != new_horz || vert_char != new_vert) {
      ...
      (maxY >= 0) ? _playerDestScreen->xyLimits.maxY = maxY
                  : _playerDestScreen->xyLimits.maxY = maxY;
    ...
    if (kbhit()) {
      ...
      if (std::tolower(k) == 'r') {
        if (_playerList.size() > 0) {
          _playerList.erase(_playerList.begin() + _destCurrPos);
          _playerProfList.erase(_playerProfList.begin() + _destCurrPos);
          _profAvailable++;
        ...
        DrawRange infoRange;
        if (_playerList.size() > 0 && _profAvailable < 1) {
          infoRange.minX = 1;
          infoRange.minY = 1;
          listNavigate();
          destNavigate();
          ...
          // createListPerimeter(*_destListScreen, _destOptions);
          // createListScreen(*_destListScreen, _destList, _destTitle,
          // _highlightDest); drawSmall(_cornerDims.minX, _cornerDims.maxX,
          // _cornerDims.minY, _cornerDims.maxY + 1, *_listScreen);
          // drawSmall(_playerDestiScreen->xyLimits.minX,
          // _playerDestScreen->xyLimits.maxX,
          //          _playerDestScreen->xyLimits.minY,
          //          _playerDestScreen->xyLimits.maxY + 1, *_playerDestScreen);
        }
...
void ListHighlightProfSelect::listNavigate() {
  color_code textCol;
  color_code bgCol;
#ifdef _WIN32
  textCol = BLUE;
...
void ListHighlightProfSelect::destNavigate() {
  color_code textCol;
  color_code bgCol;
#ifdef _WIN32
  textCol = BLUE;
  ...
  int titlePos =
      _playerDestScreen->xyLimits.maxX -
      (_playerDestScreen->xyLimits.maxX - _playerDestScreen->xyLimits.minX) /
          2 -
      _player->getName().size() / 2 - 1;
  if (titlePos < 0)
    titlePos = 0;

  for (int i = 0; i < _player->getName().size(); ++i) {
    _playerDestScreen
        ->charMap[_playerDestScreen->xyLimits.minY + 1][i + titlePos] =
        _player->getName()[i];
    _playerDestScreen
        ->colorMap[_playerDestScreen->xyLimits.minY + 1][i + titlePos] = YELLOW;
  }

  for (int i = _playerDestScreen->xyLimits.minY + 3;
       i < _playerDestScreen->xyLimits.maxY - 1; ++i) {
    // int idxOffset = _currPos - _destListScreen->xyLimits.maxY + 6;

    if (_destCurrPos > _playerDestScreen->xyLimits.maxY - 6 + _destStartIdx) {
      _destStartIdx++;
    } else if (_destCurrPos < _destStartIdx) {
      _destStartIdx = _destCurrPos;
    }

    int strdx = i - (_playerDestScreen->xyLimits.minY + 3) + _destStartIdx;
    if (strdx < 0)
      strdx = 0;
    if (strdx < _playerList.size()) {
      for (int j = 0; j < _playerList[strdx].size(); ++j) {
        if (strdx == _destCurrPos && _highlightDest) {
          _destPrevPos = _destCurrPos;
          _playerDestScreen
              ...
              _playerList[strdx][j];
          _playerDestScreen
              ->colorMap[i][j + _playerDestScreen->xyLimits.minX + 1] = textCol;
          _playerDestScreen
              ->bGColorMap[i][j + _playerDestScreen->xyLimits.minX + 1] = bgCol;
        } else {
          _playerDestScreen
              ->charMap[i][j + _playerDestScreen->xyLimits.minX + 1] =
              _playerList[strdx][j];
...
void clearPrevScreen(const std::vector<ScreenVals> &screens) {
  drawSmall(screens.back().xyLimits.minX, screens.back().xyLimits.maxX,
            screens.back().xyLimits.minY, screens.back().xyLimits.maxY + 1,
            screens.front());
}

AccessInventory::AccessInventory(std::vector<std::string> &inList,
                                 Entity &inEntity, ScreenVals &primaryScreen,
  ...
  createListScreen(*_entityListScreen, _entityInv, _player.get()->getName(),
                   false);
  // drawSmall(_cornerDims.minX, _cornerDims.maxX, _cornerDims.minY,
  // _cornerDims.maxY + 1, *_listScreen);
  drawSmall(_entityListScreen->xyLimits.minX, _entityListScreen->xyLimits.maxX,
            _entityListScreen->xyLimits.minY,
            _entityListScreen->xyLimits.maxY + 1, *_entityListScreen);
...
AccessInventory::AccessInventory() {
  ...
  std::cout << "Size of Inventory Vector:" << _inventoryList.size()
            << std::endl;
...
void AccessInventory::createInventoryList() {
  ...
  for (int i = 0; i < _inventoryList.size(); ++i) {
    for (int j = 0; j < _inventoryList.at(i).size(); ++j) {
      ...
      // printf("%s %u\n", _inventoryList.at(i).at(j)->getName().c_str(),
      // _inventoryList.at(i).at(j)->getValue().gold);
    }
  }
}

std::vector<std::vector<std::shared_ptr<Items>>>
AccessInventory::getInventoryList() {
