...
std::vector<std::string> CHR_CLASS_SKILLS::getClassSkills() {
    ...
    stringVect.push_back("Hit Die Type d" + std::to_string((int)_classTbl.HitDieType) +
                         " Max Hit Die: " + std::to_string(_classTbl.MaxHitDie));
    if (_classTbl.spellAbility) {
...
void Ranger::listClassSkills() {
    ...
    std::cout << "Non proficieny pentalty: " << _nonProfPenalty << std::endl;
    std::cout << "Added Proficiency in Weapons Per Level: 1/" << _addedWeapLvlDen << std::endl;
    std::cout << "Chance to be surprised d" << (int)_chanceToBeSurprised.sidedDie << " roll 1-"
              << (int)_chanceToBeSurprised.maxRange << std::endl;
    std::cout << "Chance to surprise d" << (int)_chanceToSurprise.sidedDie << " roll 1-"
              << (int)_chanceToSurprise.maxRange << std::endl;
    ...
    std::cout << "Track outdoors each creature above 1: +" << (int)oDTrackMoreThan1Per << "\% per creature"
              << std::endl;
...
std::vector<std::string> Ranger::getClassSkills() {
    ...
    stringVect.push_back("Non Proficiency Penalty: " + std::to_string(_nonProfPenalty));
    stringVect.push_back("Added Proficiency in Weapons Per Level: 1/" + std::to_string(_addedWeapLvlDen));
    stringVect.push_back("Chance to be surprised d" + std::to_string(_chanceToBeSurprised.sidedDie) + " roll 1-" +
                         std::to_string(_chanceToBeSurprised.maxRange));
    stringVect.push_back("Chance to surprise d" + std::to_string(_chanceToSurprise.sidedDie) + " roll 1-" +
                         std::to_string(_chanceToSurprise.maxRange));
    ...
    stringVect.push_back("Track outdoors each creature above 1: +" + std::to_string(oDTrackMoreThan1Per) +
                         "\% per creature");
    stringVect.push_back("Track outdoors every 24hours passed: " + std::to_string(oDTrack24HrPer) + "\% per day");
    stringVect.push_back("Track outdoors every hour of precipitaion: " + std::to_string(oDTrackHourPercip) +
                         "\% per hour");
    return stringVect;
}
...
void Thief::setRaceMod(RACE race) {
    ...
    _thiefTbl.pickPockets = std::clamp(_thiefTbl.pickPockets, int16_t(0), int16_t(125));
    _thiefTbl.openLocks = std::clamp(_thiefTbl.openLocks, int16_t(0), int16_t(99));
    _thiefTbl.findRemoveTraps = std::clamp(_thiefTbl.findRemoveTraps, int16_t(0), int16_t(99));
    _thiefTbl.moveSilently = std::clamp(_thiefTbl.moveSilently, int16_t(0), int16_t(99));
    _thiefTbl.hideInShadows = std::clamp(_thiefTbl.hideInShadows, int16_t(0), int16_t(99));
    _thiefTbl.hearNoise = std::clamp(_thiefTbl.hearNoise, int16_t(0), int16_t(99));
    _thiefTbl.readLanguages = std::clamp(_thiefTbl.readLanguages, int16_t(0), int16_t(99));
    _thiefTbl.climbWalls = std::clamp(_thiefTbl.climbWalls, 0.0f, 99.9f);
}
...
void Thief::setThiefSkills(RACE inRace, unsigned level) {
    ...
    default:
        if (level > 17) {
            ...
            _thiefTbl.hideInShadows = 99;
            _thiefTbl.hearNoise = 55;
            _thiefTbl.climbWalls = 99.8; // Scales slightly or caps at 99.9%
            _thiefTbl.readLanguages = 80;
        }
        break;
    }
