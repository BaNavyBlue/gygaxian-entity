...
Entity::Entity(stats inStats[2], std::string name, SEX sex, RACE race,
               std::vector<CHAR_CLASS> chrClass, ALIGNMENT align) {
  _name = name;
  _sex = sex;
  _alignment = align;
  _stats = inStats[0];
  ...
  if (_chrClass[0] == RANGER || _chrClass[0] == MONK) {
    _hitPointsBase = _curHitPoints =
        _chrSkills[0]->rollHP() + _chrSkills[0]->rollHP() + _consTbl.hpAdj;
  } else {
...
void Entity::setChrClassSkill(CHAR_CLASS inClass) {
  ...
  case THIEF:
    ...
    if (std::find(_languages.begin(), _languages.end(), THIEVES_CANT) !=
        _languages.end()) {
...
void Entity::setConsTbl() {
  ...
  case 16:
    ...
    break;
  case 17:
    if (_chrClass[0] == FIGHTER || _chrClass[0] == PALADIN ||
        _chrClass[0] == RANGER) {
      _consTbl.hpAdj = 3;
    } else {
      _consTbl.hpAdj = 2;
...
float Entity::getMeleeAttkRnd() { return _meleeAttkRound; }

stats Entity::getStats() { return _stats; }

stats Entity::getModStats() { return _modStats; }

std::string Entity::getName() { return _name; }

SEX Entity::getSex() { return _sex; }

void Entity::setRace(RACE race) { _race = race; }
...
//             _stats.excStren = _modStats.excStren = rollDice(10, true) +
//             rollDice(10, true)*10 + 1;
//         }
//         _stats.intelligence = _modStats.intelligence = bestThree();
//         _stats.wisdom = _modStats.wisdom = bestThree();
//         _stats.charisma = _stats.raceCharisma = _modStats.charisma =
//         _modStats.raceCharisma = bestThree(); _stats.dexterity =
//         _modStats.dexterity = bestThree(); _stats.constitution =
//         _modStats.constitution = bestThree();
//     }while(!rollFailure(_stats));
// }

RACE Entity::getRace() { return _race; }

ALIGNMENT Entity::getAlign() { return _alignment; }

std::vector<CHAR_CLASS> Entity::getClass() { return _chrClass; }
...
void Entity::setBaseLanguages() {
  ...
  case HUMAN:
    ...
    break;
  case ELF:
    _languages.insert(_languages.end(), {COMMON_L, ELVISH, GNOMISH, HALFLING_L,
                                         GOBLIN_L, HOBGOBLIN_L, ORCISH, GNOLL});
    break;
  case GNOME:
    _languages.insert(_languages.end(),
                      {COMMON_L, GNOMISH, DWARVISH, HALFLING_L, GOBLIN_L,
                       KOBOLD, BURROWING_ANIMALS});
    break;
  case HALF_ELF:
    _languages.insert(_languages.end(), {COMMON_L, ELVISH, GNOMISH, HALFLING_L,
                                         GOBLIN_L, HOBGOBLIN_L, ORCISH, GNOLL});
    break;
  case HALFLING:
    _languages.insert(_languages.end(), {COMMON_L, HALFLING_L, DWARVISH, ELVISH,
                                         GNOMISH, GOBLIN_L, ORCISH});
    break;
  case HALF_ORC:
  ...
  case DWARF:
    _languages.insert(_languages.end(),
                      {COMMON_L, DWARVISH, GNOMISH, GOBLIN_L, KOBOLD, ORCISH});
    break;
  }
}
...
void Entity::addWeapProf(profData prof) { _weaponProf.push_back(prof); }

strengthTable Entity::getStrTbl() { return _strTbl; }

intelligenceTable Entity::getIntTbl() { return _intTbl; }

wisdomTable Entity::getWisTbl() { return _wisTbl; }

dexterityTable Entity::getDexTbl() { return _dexTbl; }

dexThieves Entity::getDexTheif() { return _dexThief; }

constitutionTable Entity::getConsTbl() { return _consTbl; }

charismaTable Entity::getCharTbl() { return _charTbl; }

std::vector<LANGUAGE> Entity::getLanguages() { return _languages; }

money Entity::getMoney() { return _money; }

unsigned Entity::getLevel() { return _level; }

unsigned Entity::getMaxHP() { return _hitPointsBase; }

unsigned Entity::getCurHP() { return _curHitPoints; }

double Entity::getWeightAllowed() { return _weightAllowedGP; }

double Entity::getWeightCarried() { return _totalWeightGP; }

float Entity::getEquivalentGP() {
  return _money.gold + static_cast<float>(_money.copper) / 200 +
         static_cast<float>(_money.silver) / 20 +
         static_cast<float>(_money.electrum) / 2 + _money.platinum * 5;
}

short int Entity::getArmorRating() { return _armorRating; }

std::vector<unsigned> Entity::getExperience() { return _experience; }

std::shared_ptr<RACE_SKILLS> Entity::getRaceSkills() { return _raceSkills; }
...
bool Entity::isFighter() {
  for (unsigned i = 0; i < _chrClass.size(); i++) {
    if (_chrClass[i] == FIGHTER || _chrClass[i] == PALADIN ||
        _chrClass[i] == RANGER) {
      return true;
    }
  }
...
bool Entity::saveChar() {
  ...
  outTxt += "        \"weapProf\":{\r\n"; // + std::to_string(_chrClass[0]) +
                                          // ",\r\n"; // vector for multiclass
  outTxt += "            \"count\":" +
            std::to_string(static_cast<long long>(_weaponProf.size()));
  if (_weaponProf.size() > 0) {
    outTxt += ",\r\n            \"index\":{\r\n";
    for (std::size_t i = 0; i < _weaponProf.size(); ++i) {
      outTxt += "                \"" + std::to_string(i) + "\":{\r\n";
      outTxt += "                    \"armID\":" +
                std::to_string(_weaponProf[i].ARM_ID) + ",\r\n";
      outTxt += "                    \"enum\":" +
                std::to_string(_weaponProf[i].prof) + ",\r\n";
      outTxt +=
          "                    \"string\":\"" + _weaponProf[i].profS + "\"\r\n";
      outTxt += "                }";
  ...
  outTxt += "        \"experience\":{\r\n"; // + std::to_string(_chrClass[0]) +
                                            // ",\r\n"; // vector for multiclass
  outTxt +=
      "            \"count\":" + std::to_string(_experience.size()) + ",\r\n";
  outTxt += "            \"index\":{\r\n";
  for (std::size_t i = 0; i < _experience.size(); ++i) {
    outTxt += "                \"" + std::to_string(i) +
              "\":" + std::to_string(_experience[i]);
  ...
  outTxt +=
      "            \"electrum\":" + std::to_string(_money.electrum) + ",\r\n";
  outTxt +=
      "            \"platinum\":" + std::to_string(_money.platinum) + "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"inventory\":{\r\n";
  outTxt +=
      "            \"count\":" + std::to_string(_inventory.size()) + ",\r\n";
  outTxt += "            \"index\":{\r\n";
  for (std::size_t i = 0; i < _inventory.size(); ++i) {
    outTxt += "                \"" + std::to_string(i) + "\":{\r\n";
    outTxt += "                    \"itemID\":" +
              std::to_string(_inventory[i].getID()) + ",\r\n";
    outTxt += "                    \"type\":" +
              std::to_string(_inventory[i].getType()) + ",\r\n";
    outTxt += "                    \"count\":" +
              std::to_string(_inventory[i].getCount()) + "\r\n";
  ...
  outTxt += "        \"stats\":{\r\n";
  outTxt +=
      "            \"strength\":" + std::to_string(_stats.strength) + ",\r\n";
  outTxt += "            \"excStrength\":" + std::to_string(_stats.excStren) +
            ",\r\n";
  outTxt +=
      "            \"intelligence\":" + std::to_string(_stats.intelligence) +
      ",\r\n";
  outTxt += "            \"wisdom\":" + std::to_string(_stats.wisdom) + ",\r\n";
  outTxt +=
      "            \"dexterity\":" + std::to_string(_stats.dexterity) + ",\r\n";
  outTxt +=
      "            \"constitution\":" + std::to_string(_stats.constitution) +
      ",\r\n";
  outTxt +=
      "            \"charisma\":" + std::to_string(_stats.charisma) + ",\r\n";
  outTxt +=
      "            \"raceCharisma\":" + std::to_string(_stats.raceCharisma) +
      "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"modStats\":{\r\n";
  outTxt +=
  ...
  outTxt +=
      "            \"hitProb\":" + std::to_string(_strTbl.hitProb) + ",\r\n";
  outTxt += "            \"damageAdj\":" + std::to_string(_strTbl.damageAdj) +
            ",\r\n";
  outTxt += "            \"weightAllowMod\":" +
            std::to_string(_strTbl.weightAllowMod) + ",\r\n";
  outTxt += "            \"openDoors\":" + std::to_string(_strTbl.openDoors) +
            ",\r\n";
  outTxt += "            \"bendBarsLiftGatesPer\":" +
            std::to_string(_strTbl.bendBarsLiftGatesPer) + "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"wisTable\":{\r\n";
  outTxt += "            \"magicAttackAdj\":" +
            std::to_string(_wisTbl.magicAttackAdj) + ",\r\n";
  outTxt += "            \"spellBonus\":" + std::to_string(_wisTbl.spellBonus) +
            ",\r\n";
  outTxt +=
      "            \"spellFailPer\":" + std::to_string(_wisTbl.spellFailPer) +
      "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"intTable\":{\r\n";
  outTxt += "            \"chanceToKnowPer\":" +
            std::to_string(_intTbl.chanceToKnowPer) + ",\r\n";
  outTxt += "            \"minimumSpellsPer\":" +
            std::to_string(_intTbl.minumumSpellsPerLevel) + ",\r\n";
  outTxt += "            \"maximumSpellsPer\":" +
            std::to_string(_intTbl.maxiumSpellsPerlevel) + ",\r\n";
  outTxt += "            \"possibleAddLang\":" +
            std::to_string(_intTbl.possibAddLang) + "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"dexTable\":{\r\n";
  outTxt +=
      "            \"reactAttkAdj\":" + std::to_string(_dexTbl.reactAttkAdj) +
      ",\r\n";
  outTxt += "            \"defenseAdj\":" + std::to_string(_dexTbl.defenseAdj) +
            "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"dexThief\":{\r\n";
  outTxt += "            \"pickPocketPer\":" +
            std::to_string(_dexThief.pickPocketPer) + ",\r\n";
  outTxt += "            \"openLocks\":" + std::to_string(_dexThief.openLocks) +
            ",\r\n";
  outTxt +=
      "            \"locRemTraps\":" + std::to_string(_dexThief.locRemTraps) +
      ",\r\n";
  outTxt += "            \"movSilent\":" + std::to_string(_dexThief.movSilent) +
            ",\r\n";
  outTxt += "            \"hideInShadows\":" +
            std::to_string(_dexThief.hideInShadows) + "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"consTbl\":{\r\n";
  outTxt += "            \"hpAdj\":" + std::to_string(_consTbl.hpAdj) + ",\r\n";
  outTxt += "            \"sysShockSurPer\":" +
            std::to_string(_consTbl.sysShockSurPer) + ",\r\n";
  outTxt +=
      "            \"resurSurvPer\":" + std::to_string(_consTbl.resurSurvPer) +
      "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"charTbl\":{\r\n";
  outTxt +=
      "            \"maxHenchMen\":" + std::to_string(_charTbl.maxHenchMen) +
      ",\r\n";
  outTxt += "            \"loyaltyBasePer\":" +
            std::to_string(_charTbl.loyaltyBasePer) + ",\r\n";
  outTxt += "            \"reactAdjustPer\":" +
            std::to_string(_charTbl.reactAdjustPer) + "\r\n";
  outTxt += "        },\r\n";
  outTxt += "        \"languages\":{\r\n";
  outTxt +=
      "            \"count\":" + std::to_string(_languages.size()) + ",\r\n";
  outTxt += "            \"index\":{\r\n";
  for (unsigned i = 0; i < _languages.size(); ++i) {
    outTxt += "                \"" + std::to_string(i) +
              "\":" + std::to_string(_languages[i]);
  ...
  if (myFile.is_open()) {
    ...
    std::cout << "File Can't open path " << filename
              << " invalid or cannot be accessed." << std::endl;
    return false;
  }
  return true;
}
...
void Entity::loadEntity(std::string file) {
  ...
  _strTbl.bendBarsLiftGatesPer =
      uint64_t(strengthTable["bendBarsLiftGatesPer"]);

  _intTbl.chanceToKnowPer =
      uint64_t(charData["data"]["intTable"]["chanceToKnowPer"]);
  _intTbl.minumumSpellsPerLevel =
      uint64_t(charData["data"]["intTable"]["minimumSpellsPer"]);
  _intTbl.maxiumSpellsPerlevel =
      uint64_t(charData["data"]["intTable"]["maximumSpellsPer"]);
  _intTbl.possibAddLang =
      uint64_t(charData["data"]["intTable"]["possibleAddLang"]);

  _wisTbl.magicAttackAdj =
      int64_t(charData["data"]["wisTable"]["magicAttackAdj"]);
  ...
  _dexThief.pickPocketPer =
      int64_t(charData["data"]["dexThief"]["pickPocketPer"]);
  ...
  _dexThief.hideInShadows =
      int64_t(charData["data"]["dexThief"]["hideInShadows"]);

  _consTbl.hpAdj = int64_t(charData["data"]["consTbl"]["hpAdj"]);
  _consTbl.sysShockSurPer =
      uint64_t(charData["data"]["consTbl"]["sysShockSurPer"]);
  ...
  _charTbl.loyaltyBasePer =
      int64_t(charData["data"]["charTbl"]["loyaltyBasePer"]);
  _charTbl.reactAdjustPer =
      int64_t(charData["data"]["charTbl"]["reactAdjustPer"]);

  for (int i = 0; i < int64_t(charData["data"]["languages"]["count"]); ++i) {
    std::string idx = std::to_string(i);
    _languages.push_back(
        (LANGUAGE)(uint64_t(charData["data"]["languages"]["index"][idx])));
  }
  ...
  for (int i = 0; i < int64_t(charData["data"]["class"]["count"]); ++i) {
    std::string idx = std::to_string(i);
    _chrClass.push_back(
        (CHAR_CLASS)(uint64_t(charData["data"]["class"]["index"][idx])));
  }

  printf("Just before weapProf\n");
  if (uint64_t(charData["data"]["weapProf"]["count"]) > 0) {
    for (int i = 0; i < uint64_t(charData["data"]["weapProf"]["count"]); ++i) {
      std::string idx = std::to_string(i);
      profData prof;
      prof = profDat[(
          int64_t(charData["data"]["weapProf"]["index"][idx]["enum"]))];
  ...
  for (int i = 0; i < int64_t(charData["data"]["experience"]["count"]); ++i) {
    std::string idx = std::to_string(i);
    _experience.push_back(
        (uint64_t(charData["data"]["experience"]["index"][idx])));
  }
  ...
  // std::cout << "\r\nWeight Allowed in gold pieces (gp): " <<
  // getWeightAllowed() << std::endl; std::cout << "\r\nTotal Weight gp: " <<
  // getWeightCarried() << std::endl; printMoney(_money); std::cout << "\r\nMod
  // Stats: "; printStats(_modStats); printStrTbl(_strTbl);
  // printIntTbl(_intTbl);
  // printWisTbl(_wisTbl);
  // printDexTbl(_dexTbl);
...
Entity::Entity(const char *filename) { loadEntity(filename); }
...
std::vector<profData> Entity::getWeapProf() { return _weaponProf; }

void Entity::setWeaponProf(profData newProf) { _weaponProf.push_back(newProf); }

std::vector<Items> Entity::getInventory() { return _inventory; }
