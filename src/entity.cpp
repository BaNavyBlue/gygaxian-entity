...
Entity::Entity(stats inStats[2], std::string name, SEX sex, RACE race, std::vector<CHAR_CLASS> chrClass,
               ALIGNMENT align) {
    _name = name;
    _sex = sex;
    _alignment = align;
...
void Entity::setBaseLanguages() {
    ...
    case ELF:
        _languages.insert(_languages.end(),
                          {COMMON_L, ELVISH, GNOMISH, HALFLING_L, GOBLIN_L, HOBGOBLIN_L, ORCISH, GNOLL});
        break;
    case GNOME:
        _languages.insert(_languages.end(),
                          {COMMON_L, GNOMISH, DWARVISH, HALFLING_L, GOBLIN_L, KOBOLD, BURROWING_ANIMALS});
        break;
    case HALF_ELF:
        _languages.insert(_languages.end(),
                          {COMMON_L, ELVISH, GNOMISH, HALFLING_L, GOBLIN_L, HOBGOBLIN_L, ORCISH, GNOLL});
...
float Entity::getEquivalentGP() {
    return _money.gold + static_cast<float>(_money.copper) / 200 + static_cast<float>(_money.silver) / 20 +
...
bool Entity::saveChar() {
    ...
    outTxt += "        \"weapProf\":{\r\n"; // + std::to_string(_chrClass[0]) +
                                            // ",\r\n"; // vector for multiclass
    outTxt += "            \"count\":" + std::to_string(static_cast<long long>(_weaponProf.size()));
    ...
    outTxt += "        },\r\n";
    outTxt += "        \"modStats\":{\r\n";
    outTxt += "            \"strength\":" + std::to_string(_modStats.strength) + ",\r\n";
    outTxt += "            \"excStrength\":" + std::to_string(_modStats.excStren) + ",\r\n";
    outTxt += "            \"intelligence\":" + std::to_string(_modStats.intelligence) + ",\r\n";
    outTxt += "            \"wisdom\":" + std::to_string(_modStats.wisdom) + ",\r\n";
    outTxt += "            \"dexterity\":" + std::to_string(_modStats.dexterity) + ",\r\n";
    outTxt += "            \"constitution\":" + std::to_string(_modStats.constitution) + ",\r\n";
    outTxt += "            \"charisma\":" + std::to_string(_modStats.charisma) + ",\r\n";
    outTxt += "            \"raceCharisma\":" + std::to_string(_modStats.raceCharisma) + "\r\n";
    outTxt += "        },\r\n";
...
void Entity::loadEntity(std::string file) {
    ...
    _modStats.excStren = uint64_t(modStats["excStrength"]);
    ...
    if (uint64_t(charData["data"]["weapProf"]["count"]) > 0) {
        for (int i = 0; i < uint64_t(charData["data"]["weapProf"]["count"]); ++i) {
            std::string idx = std::to_string(i);
            profData prof;
            prof = profDat[(int64_t(charData["data"]["weapProf"]["index"][idx]["enum"]))];
