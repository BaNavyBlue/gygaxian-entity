...
RACE_SKILLS::~RACE_SKILLS() {
    // std::cout << "In RACE_SKILLS destructor" << std::endl;LF
}
...
void Dwarf::listRaceSkills() {
    std::cout << "Detect Grade or Slope roll d" << (int)detectGrade.sidedDie << " succeeds: 1-"
              << (int)detectGrade.maxRange << std::endl;
    std::cout << "Detect recent Construction passage or tunnel roll d" << (int)dtctConstrPsg.sidedDie << " succeeds: 1-"
              << (int)dtctConstrPsg.maxRange << std::endl;
    std::cout << "Detect shifting or sliding walls roll d" << (int)dtctMovingWalls.sidedDie << " succeeds: 1-"
              << (int)dtctMovingWalls.maxRange << std::endl;
    std::cout << "Detect traps inv. pits blocks or stonework roll d" << (int)dtctTraps.sidedDie << " succeeds: 1-"
              << (int)dtctTraps.maxRange << std::endl;
...
std::vector<std::string> Dwarf::getRaceSkills() {
    ...
    skillList.push_back("Detect Grade or Slope roll d" + std::to_string(detectGrade.sidedDie) + " succeeds: 1-" +
                        std::to_string(detectGrade.maxRange));
    skillList.push_back("Detect recent Construction passage or tunnel roll d" + std::to_string(dtctConstrPsg.sidedDie) +
                        " succeeds: 1-" + std::to_string(dtctConstrPsg.maxRange));
    skillList.push_back("Detect shifting or sliding walls roll d" + std::to_string(dtctMovingWalls.sidedDie) +
                        " succeeds: 1-" + std::to_string(dtctMovingWalls.maxRange));
    skillList.push_back("Detect traps inv. pits blocks or stonework roll d" + std::to_string(dtctTraps.sidedDie) +
                        " succeeds: 1-" + std::to_string(dtctTraps.maxRange));
...
void Elf::setRaceSkills(stats inStats) {
    ...
    chanceToSurprisePort.maxRange = 2;
}

void Elf::listRaceSkills() {
    std::cout << "Resist sleep or charm d" << (int)resistSlpChrm.sidedDie << " succeeds: 1-"
              << (int)resistSlpChrm.maxRange << std::endl;
    std::cout << "Detect secret or concealed door passive within 10' d" << (int)secConDoorPsv.sidedDie
              << " succeeds: 1-" << (int)secConDoorPsv.maxRange << std::endl;
    std::cout << "Detect secret or concealed door active d" << (int)secConDoorActv.sidedDie << " succeeds: 1-"
              << (int)secConDoorActv.maxRange << std::endl;
...
void Gnome::listRaceSkills() {
    std::cout << "Detect Grade or Slope roll d" << (int)detectGrade.sidedDie << " succeeds: 1-"
              << (int)detectGrade.maxRange << std::endl;
    std::cout << "Detect unsafe walls or ceilings d" << (int)dtctUnsafeWCF.sidedDie << " succeeds: 1-"
              << (int)dtctUnsafeWCF.maxRange << std::endl;
    std::cout << "Determine direction of travel underground d" << (int)detDirecUG.sidedDie << " succeeds: 1-"
              << (int)detDirecUG.maxRange << std::endl;
    std::cout << "Detect depth bellow ground roll d" << (int)dtctDepth.sidedDie << " succeeds: 1-"
              << (int)dtctDepth.maxRange << std::endl;
    std::cout << "Infravision up to " << infraVision << " feet" << std::endl;
    std::cout << "To hit bonus while fighting Half-Orc, Goblin, Hobgoblin and Orc +" << toHitBonus << std::endl;
...
std::vector<std::string> Gnome::getRaceSkills() {
    ...
    skillList.push_back("Detect Grade or Slope roll d" + std::to_string(detectGrade.sidedDie) + " succeeds: 1-" +
                        std::to_string(detectGrade.maxRange));
    skillList.push_back("Determine direction of travel underground d" + std::to_string(detDirecUG.sidedDie) +
                        " succeeds: 1-" + std::to_string(detDirecUG.maxRange));
    skillList.push_back("Detect unsafe walls or ceilings d" + std::to_string(dtctUnsafeWCF.sidedDie) + " succeeds: 1-" +
                        std::to_string(dtctUnsafeWCF.maxRange));
    skillList.push_back("Detect depth bellow ground roll d" + std::to_string(dtctDepth.sidedDie) + " succeeds: 1-" +
                        std::to_string(dtctDepth.maxRange));
    skillList.push_back("Infravision up to " + std::to_string(infraVision) + "'");
    skillList.push_back("To hit bonus while fighting Half-Orc, Goblin, Hobgoblin and Orc +" +
...
void Halfling::listRaceSkills() {
    std::cout << "Detect Grade or Slope roll d" << (int)detectGrade.sidedDie << " succeeds: 1-"
              << (int)detectGrade.maxRange << std::endl;
    std::cout << "Determine direction of travel d" << (int)detDirecUG.sidedDie << " succeeds: 1-"
              << (int)detDirecUG.maxRange << std::endl;
    std::cout << "Chance to surprise creatures in a party of all Elves, or Elves and Halflings d"
              << (int)chanceToSurprise.sidedDie << " succeeds: 1-" << (int)chanceToSurprise.maxRange << std::endl;
    std::cout << "Infravision up to " << infraVision << " feet" << std::endl;
