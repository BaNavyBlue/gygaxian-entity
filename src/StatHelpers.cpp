...
RACE inputRace(stats inStats, std::vector<std::string> &viable, std::vector<std::string> &nonViable) {
    // OS Level Write to standard Out
    // std::vector<std::string> races = {"Human: 0", "Elf: 1", "Half-Orc: 2", "Halfling: 3", "Dwarf: 4", "Half-Elf: 5",
    // "Gnome: 6"};
...
ALIGNMENT inputAlign(CHAR_CLASS inClass) {
    // OS Level Write to standard Out
    // std::vector<std::string> races = {"Human: 0", "Elf: 1", "Half-Orc: 2", "Halfling: 3", "Dwarf: 4", "Half-Elf: 5",
    // "Gnome: 6"}; std::vector<racePair> racePairs = {{HUMAN, "Human"}, {ELF, "Elf"}, {HALF_ORC, "Half-Orc"}, {DWARF,
    // "Dwarf"}, {HALFLING, "Halfling"}, {HALF_ELF, "Half-Elf"}, {GNOME, "Gnome"}};
    std::string message = "\r\nSelect Alignment: \r\n";
...
std::vector<std::string> getDexTheif(dexThieves dexThief) {
    ...
    basicList.push_back("Picking Pockets: " + std::to_string(dexThief.pickPocketPer) + '%');
    basicList.push_back("Opening Locks: " + std::to_string(dexThief.openLocks) + '%');
    basicList.push_back("Locating/Removing Traps: " + std::to_string(dexThief.locRemTraps) + '%');
    basicList.push_back("Moving Silently: " + std::to_string(dexThief.movSilent) + '%');
    basicList.push_back("Hiding In Shadows: " + std::to_string(dexThief.hideInShadows) + '%');
    return basicList;
}
...
std::vector<std::string> getConsTbl(constitutionTable consTbl) {
    ...
    basicInfo.push_back("System Shock Survivial: " + std::to_string(consTbl.sysShockSurPer) + '%');
    basicInfo.push_back("Resurection Survival: " + std::to_string(consTbl.resurSurvPer) + '%');
    return basicInfo;
}
...
std::vector<std::string> getCharTbl(charismaTable charTbl) {
    ...
    basicInfo.push_back("Loyalty Percentage: " + std::to_string(charTbl.loyaltyBasePer) + '%');
    basicInfo.push_back("Reaction of others: " + std::to_string(charTbl.reactAdjustPer) + '%');
    return basicInfo;
}
...
bool checkClassStats(CHAR_CLASS inClass, stats inStats) {
    ...
    case PALADIN:
        ...
        if (inStats.strength < 12 || inStats.intelligence < 9 || inStats.wisdom < 13 || inStats.constitution < 9 ||
            inStats.charisma < 17) {
            return false;
        }
...
//        return false;
//    }
//    if (inStats.intelligence < 6 && (inStats.strength < 9 || inStats.constitution < 7)) {
...
bool rollFailure(stats inStats, bool printMsg) {
    bool atLeastOneClassViable = false;

    // Check every class in your enum
    for (int i = 0; i <= MONK; i++) {
        if (classStatCheck(static_cast<CHAR_CLASS>(i), inStats)) {
            atLeastOneClassViable = true;
            break; // We found one! This roll is "legal."
        }
    }
    ...
    return true; // Character is playable
}
...
std::vector<std::string> getStrTbl(strengthTable strTbl) {
    ...
    basicInfo.push_back("Bend Bars / Lift Gates: " + std::to_string(strTbl.bendBarsLiftGatesPer) + '%');
    return basicInfo;
}
...
std::vector<std::string> getIntTbl(intelligenceTable intTbl) {
    ...
    basicInfo.push_back("Chance to Know each Listed Spell: " + std::to_string(intTbl.chanceToKnowPer) + '%');
...
std::vector<std::string> getWisTbl(wisdomTable wisTbl) {
    ...
    basicInfo.push_back("Chance for Spell Failure: " + std::to_string(wisTbl.spellFailPer) + '%');
    return basicInfo;
}
